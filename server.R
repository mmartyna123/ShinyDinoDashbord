...
palettemap = c('#98FB98', '#00FF7F', '#32CD32', '#228B22', '#006400')
...
server <- function(input, output, session) {
  ...
  output$total_types <- renderValueBox({
    valueBox(value = length(stri_omit_empty(unique(dino_data$type), na_empty = FALSE)), subtitle = "Number of Dinosaur Types", icon = icon("paw"), color = "blue")
  })
  
  output$total_periods <- renderValueBox({
    valueBox(value = length(stri_omit_empty(unique(dino_data$period_name), na_empty = FALSE)), subtitle = "Number of Geological Periods", icon = icon("clock"), color = "green")
  })
  ...
  output$map <- renderLeaflet({
    ...
    color_palette <- colorBin(palette = palettemap, domain = world$count, bins = 5, na.color = "transparent")
